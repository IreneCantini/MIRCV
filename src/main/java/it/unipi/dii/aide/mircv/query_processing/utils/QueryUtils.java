...
public class QueryUtils {
    ...
    private DictionaryElem dictionaryBinarySearch(String term) throws IOException {
        ...
        long low = 0;
        long high = (RandomAccessFile_map.get(SPIMI.block_number+1).get(0).getChannel().size() / 68); // Mantiene quanti termini sono presenti nel dizionario
        long mid;
        ...
        while (low <= high) {
            ...
            // get term
            mappedByteBuffer = RandomAccessFile_map.get(SPIMI.block_number+1).get(0).getChannel().map(FileChannel.MapMode.READ_ONLY, mid*68, 20);
            ...
            if (temp.compareTo(term) < 0) {
                ...
                // Termine trovato
                d.readDictionaryElemFromDisk(mid*68, RandomAccessFile_map.get(SPIMI.block_number+1).get(0).getChannel());
